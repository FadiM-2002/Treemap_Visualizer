...
class PaperTree(TMTree):
    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        <by_year> indicates whether the first level of subtrees should be
        ...
        """
        if all_papers:
            subtrees.extend(_build_tree_from_dic(_load_papers_to_dict(by_year)))
        TMTree.__init__(self, name, subtrees, citations)
        self._authors = authors
        self._doi = doi

    def get_separator(self) -> str:
        """Return the categories' separator.
        """
        return " > "

    def get_suffix(self) -> str:
        """Return the description of this paper.
        """
        if not self._subtrees:
            return f' ({self._authors})'
        return f' ({len(self._subtrees)} subtrees)'


def _load_papers_to_dict(by_year: bool = True) -> List[Dict]:
    ...
    with open(DATA_FILE) as file:
        ...
        if not by_year:
            for line in papers:
                _helper_function(line["Category"].split(": "), dic[0], line)
        else:
            for line in papers:
                if line["Year"] not in dic[0]:
                    dic[0][line["Year"]] = [{}]
                dic2 = dic[0][line["Year"]][0]
                _helper_function(line["Category"].split(": "), dic2, line)
    return dic


def _helper_function(categories: str, dic: Dict, line: Dict) -> None:
    """Helper function for the function _load_papers_to_dict.
    """
    for category in categories:
        ...
        if category != categories[-1]:
            dic = dic[category][0]
    dic[categories[-1]].append(line)


def _build_tree_from_dic(nested_dict: List[Dict]) -> List[PaperTree]:
    """Return a list of trees from the nested dictionary <nested_dict>.
    """
    trees = []
    for item in nested_dict[1:]:
        trees.append(PaperTree(item["Title"], [], item["Author"], item["Url"],
                               int(item["Citations"])))
    for dic in nested_dict[0]:
        trees.append(PaperTree(dic, _build_tree_from_dic(nested_dict[0][dic])))
    return trees
